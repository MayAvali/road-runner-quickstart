...
public final class ThreeDeadWheelLocalizer implements Localizer {
    public static class Params {
        public double par0YTicks = 1688.3867920478417; // y position of the first parallel encoder (in tick units)
        public double par1YTicks = -2106.051520917759; // y position of the second parallel encoder (in tick units)
        public double perpXTicks = -1829.3084123211668; // x position of the perpendicular encoder (in tick units)
    }
    ...
    public PoseVelocity2d update() {
        ...
        Twist2dDual<Time> twist = new Twist2dDual<>(
                new Vector2dDual<>(
                        ...
                        new DualNum<Time>(new double[] {
                                (PARAMS.perpXTicks* perpScalar / (PARAMS.par0YTicks - PARAMS.par1YTicks) * (par1PosDelta - par0PosDelta) + perpPosDelta* perpScalar),
                                (PARAMS.perpXTicks* perpScalar / (PARAMS.par0YTicks - PARAMS.par1YTicks) * (par1PosVel.velocity - par0PosVel.velocity) + perpPosVel.velocity* perpScalar),
                        }).times(inPerTick)
                ),
